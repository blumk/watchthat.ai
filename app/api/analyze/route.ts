...
import { startTrace, scheduleFlush } from "@/lib/observability";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    trace.end({ output: parsed ?? { parseOk: false } });
    ...
    // Best-effort buffer drain — happens after the response is sent so it
    // doesn't add latency. No-op when Langfuse is disabled.
    scheduleFlush();
  }
}
