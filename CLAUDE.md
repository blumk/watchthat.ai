...
**Fetching:** `POST /api/scrape` returns `{ snapshot, cached, newChange }`. It normalizes the URL, upserts a shared `pages` row, and either (a) returns the existing latest snapshot if the page was fetched within 60s (`cached: true`, no Firecrawl call), or (b) calls Firecrawl, hashes the markdown via sha256, short-circuits if the hash matches the previous snapshot (bumps `last_fetched_at`, no insert), or (c) uploads the screenshot to Supabase Storage, calls `describeChange` in-process to populate change description/classification/emoji, inserts a new `snapshots` row, and updates `pages.latest_snapshot_id`. `force: true` skips the 60s dedup. Firecrawl SDK v4 uses `.scrape()`, throws on error, returns `Document` directly. Full-page screenshot via `actions: [{ type: "screenshot", fullPage: true }]`; result at `result.actions.screenshots[0]`.
