...
- **Hash-equal re-fetch:** new snapshot row still inserts, but `/api/describe-change` is skipped (saves tokens); `change_classification` is `"quiet"`, `change_description` is `null`, response `newChange: false` [`scrape.test.ts`]
- **Hash-different re-fetch:** `/api/describe-change` is called server-side to populate `change_description` / `change_classification` / `change_emoji`; response `newChange: true` [`scrape.test.ts`]
