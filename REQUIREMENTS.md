...
- A blur with no change is a no-op (no `updateSite` call, no `onUpdate`) [`WatchedSites.test.tsx`]
...
- `getSites` resolves the watch's `watchTarget` string against the latest snapshot's fact bag (see `lib/watch-target-match`). On match, `site.trackedFact = { key, value, displayName }` and history entries whose snapshot's fact-bag value for that key differs from the last-seen value are annotated with `trackedDelta = { displayName, before?, after }`. The earliest entry per page additionally folds the first-seen tracked value directly into its description — "Initial snapshot taken with rating 4.5." instead of a generic prefix on a generic message. Unmatched targets leave both fields unset so the UI falls back to its normal rendering [`db.test.ts`, `watch-target-match.test.ts`]
...
- **Structured-data fact bag:** the scrape route also asks Firecrawl for `rawHtml` and runs `extractFacts` (see `lib/facts`) to pull a safelisted projection of JSON-LD + OpenGraph/Twitter meta into a flat `{ "Type.path": "value" }` bag. The bag is folded into the content hash via `sha256(markdown + "\n--\n" + factsBlob)` so 4.5 → 4.4 or 1217 → 1243 flips the hash even when rendered markdown rounds them away. The diff between the prev and new bags is passed to describeChange as OPTIONAL precision context — the prompt instructs Claude to use a metadata value only when it adds resolution to something already visible in the markdown (e.g. page says "2.5k" → metadata shows "2523"), and never to introduce a metadata number that has no visible counterpart. Stored on the snapshot row as `facts jsonb`. [`scrape.test.ts`, `facts.test.ts`]
...
- **User refinement notes reach describeChange:** alongside `watch_target`, the route also queries each watcher's `target_notes` (free-form text the user writes after seeing the AI get the page wrong), de-dups and filters nulls, and forwards as `userNotes`. The prompt treats these as *authoritative* — Claude is told to follow them literally over its own interpretation of the page. Lets users override hallucinations by saying e.g. "the GA price is the $ value right under '### General Admission' in the listings section; ignore JSON-LD aggregate prices." [`scrape.test.ts`]
